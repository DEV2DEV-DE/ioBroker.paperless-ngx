...
class directorieshandlerClass {
	...
	async checkDirectory(startDirectory,activeDirectories){
		const activeFunction = "checkDirectory";
		this.adapter.log.debug(`Function ${activeFunction} started.`);
		try{
			for(const adapterObject of Object.values(this.adapterObjects)){
				if(adapterObject._id.indexOf(`${this.adapter.namespace}.${startDirectory}.`) === 0){
					const idWithoutStart = adapterObject._id.substring(`${this.adapter.namespace}.${startDirectory}.`.length,adapterObject._id.length);
					let fistFolder = idWithoutStart;
					const indexOfFistDot = idWithoutStart.indexOf(".");
					if(indexOfFistDot !== -1){
						fistFolder = idWithoutStart.substring(0,indexOfFistDot);
					}
					if(!activeDirectories[fistFolder]){
						this.adapter.log.error(adapterObject._id);
						this.adapter.log.error(idWithoutStart);
						this.adapter.log.error(fistFolder);
