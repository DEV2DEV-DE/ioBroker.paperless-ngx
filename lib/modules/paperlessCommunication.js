		...
		try{
			...
			if(erg.data){
				for(const tag of erg.data.results){
					this.directoryhandler.generateRekursivObjects(tag,`${this.tags.startDirectory}.${tag.name}`);
				}
			}
		}
		catch(e){
		...
		try{
			...
			if(erg.data){
				for(const document of erg.data.results){
					this.directoryhandler.generateRekursivObjects(document,`${this.documents.startDirectory}.${document.title}`);
				}
			}
		}
		catch(e){
